...
def main():  # pragma: no cover - 集成路径,核心逻辑见 update_once
    ...
    limiter = RateLimiter(1, 2)  # 礼貌限速:每2秒≤1次(避免触发IP限流)
    fetcher = TencentMinuteFetcher(limiter=limiter)

    engine = make_engine()
    factory = make_session_factory(engine)
    print(f"[{_now_bj()}] minute_updater daemon started (Tencent)", flush=True)
